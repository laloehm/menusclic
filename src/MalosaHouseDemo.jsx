...
export default function MalosaHouseDemo() {
  ...
  const [contactInfo, setContactInfo] = useState({
    phone: '5571491246',
    whatsapp: '525571491246',
    facebook: 'https://www.facebook.com/malosa16',
  ...
  useEffect(() => {
    ...
    const unsubscribeContact = onSnapshot(doc(db, 'malosahouse_settings', 'contact'), (snapshot) => {
      if (snapshot.exists()) {
    ...
    <div className="text-malosa-primary selection:bg-malosa-secondary-container font-malosa min-h-screen bg-malosa-bg pb-12 text-lg">
      ...
      <main className="pt-28 pb-12 px-6 max-w-xl mx-auto">
        ...
        <div className="mt-10 malosa-brutalist-border bg-malosa-primary text-malosa-on-primary p-6 flex flex-col items-center text-center gap-4">
          ...
          {contactInfo.phone && (
            <a className="font-malosa text-[32px] hover:underline transition-all font-bold" href={`tel:${contactInfo.phone}`}>{contactInfo.phone}</a>
          )}
          {contactInfo.schedule && (
            <p className="font-malosa text-sm mt-2">Horario: {contactInfo.schedule}</p>
          )}
        </div>
      </main>
      ...
      <footer className="w-full py-6 mb-16 border-t-4 border-double border-malosa-primary flex flex-col items-center space-y-4 px-6 text-center bg-malosa-bg">
        ...
        <div className="flex flex-col items-center gap-4 my-4">
          {contactInfo.facebook && (
            <a className="font-malosa text-malosa-primary opacity-90 hover:opacity-100 underline transition-all" href={contactInfo.facebook} target="_blank" rel="noreferrer">Facebook: Malosa House</a>
          )}
          {contactInfo.instagram && (
            <a className="font-malosa text-malosa-primary opacity-90 hover:opacity-100 underline transition-all" href={contactInfo.instagram} target="_blank" rel="noreferrer">Instagram: Malosa House</a>
          )}
          {contactInfo.maps && (
            <a className="font-malosa text-malosa-primary opacity-90 hover:opacity-100 underline transition-all font-bold" href={contactInfo.maps} target="_blank" rel="noreferrer">Ubicación</a>
          )}
        </div>
